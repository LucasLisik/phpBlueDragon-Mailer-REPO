<h1><?php echo '<img src="'.base_url('library/haccount.png').'" width="32" height="32" style="vertical-align: middle;" />'; ?> <?php echo $this->lang->line('sends_accountsh5'); ?></h1>

<?php

if($GroupDeleted)
{
    echo '<div class="goodAction">'.$this->lang->line('sends_deleted5').'</div>';	
}

if($GroupAdded)
{
    echo '<div class="goodAction">'.$this->lang->line('sends_accountwasadded5').'</div>';	
}
...
if($LICENSE_IsLicenseExists)
{
	$CanAddNew = true;
}
else
{
	if($LICENSE_CountAccounts >= 1)
	{
		$CanAddNew = false;
	}
	else
...
if($CanAddNew)
{
	...
	echo '<h2>'.$this->lang->line('sends_addnewaccount5').'</h2>';

	echo form_open('sends');
	...
	$SubmitButton = array('name' => 'zaloguj', 'value' => $this->lang->line('sends_addaccountsubmit6'), 'style' => 'width: 100%;');
...
else
{
	...
	echo $this->lang->line('main_nolicensebrief');
}
echo '</div>';
//echo '--'.$Vsend_auth.'--';
...
?>
<script>
if($("input[name='send_auth']").is(':checked')) 
{
